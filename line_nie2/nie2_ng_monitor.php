<?php
    ...
    // NG mode columns come from tb_ng_list where Process = 'All' (same source
    // used by nie2_proc06.php's inspection NG-mode dropdown).
    $ngModeList = [];
    $nmStmt = mysqli_prepare($conn, "SELECT NGmode FROM tb_ng_list WHERE Process = 'All' ORDER BY NGmode");
    ...
    // Build a pivoted per-BoxNo query: one SUM(CASE WHEN inspNGmode = ? ...)
    // column per NG mode, plus a running total, grouped down to BoxNo level
    // so the group-level (ProdName+InvNo+WO) totals can be rolled up in PHP.
    $aliasToMode = [];
    $selectParts = ['ProdName', 'InvNo', 'WO', 'BoxNo'];
    $bindTypes   = '';
    $bindValues  = [];
    ...
    foreach ($ngModeList as $i => $mode) {
        ...
        $selectParts[] = "SUM(CASE WHEN inspNGmode = ? THEN inspNGqty ELSE 0 END) AS `$alias`";
        $bindTypes   .= 's';
        $bindValues[] = $mode;
    }
    $selectParts[] = "SUM(inspNGqty) AS TotalNG";
    ...
    $sql = "SELECT " . implode(', ', $selectParts) . " FROM tb_proc6";
    if ($where) {
        $sql .= " WHERE " . implode(' AND ', $where);
    }
    $sql .= " GROUP BY ProdName, InvNo, WO, BoxNo ORDER BY ProdName, InvNo, WO, BoxNo";
    ...
    // Roll the per-BoxNo rows up into one group per ProdName+InvNo+WO.
    $groups = [];
    foreach ($boxRows as $r) {
        $key = $r['ProdName'] . "\x1F" . $r['InvNo'] . "\x1F" . $r['WO'];
        if (!isset($groups[$key])) {
            $groups[$key] = [
                'ProdName' => $r['ProdName'],
                'InvNo'    => $r['InvNo'],
...
<body>
  ...
  <div class="ngmon-wrap">
    <h2>ติดตาม NG แต่ละ Lot / Box — Ni-e Line 2</h2>
